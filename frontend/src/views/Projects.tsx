...
const STATUS_PILL: Record<string, { background: string; color: string }> = {
  active:  { background: '#14532d', color: '#16a34a' },
  paused:  { background: '#1c1a07', color: '#d97706' },
  done:    { background: '#18181b', color: '#52525b' },
  dropped: { background: '#18181b', color: '#3f3f46' },
}
...
function UrgencyBadge({ urgency }: { urgency?: number | null }) {
  ...
  let color = '#52525b'
  if (urgency > 6) color = '#ef4444'
  else if (urgency > 3) color = '#f97316'
  else if (urgency > 1.5) color = '#d97706'
  else color = '#16a34a'
  return (
    <span className="text-xs font-mono shrink-0" style={{ color }} title={`urgency: ${urgency.toFixed(2)}`}>
      ↑{urgency.toFixed(1)}
    </span>
  )
}
    ...
    <div style={{ borderBottom: '1px solid #18181b' }}>
      ...
      >
        ...
        <button
          ...
          style={{ color: task.status === 'done' ? '#52525b' : '#e4e4e7', textDecoration: task.status === 'done' ? 'line-through' : 'none' }}
        ...
        <span className="w-14 text-right shrink-0" style={{ color: '#52525b' }}>{task.estimated_minutes}m</span>
        {task.deadline && (
          <span className="text-amber-400 shrink-0">{task.deadline.slice(0, 10)}</span>
        )}
        {task.recurrence_rule && (
          <span className="shrink-0" style={{ color: '#52525b' }}>↻</span>
        )}
        {task.dependency_ids && task.dependency_ids.length > 0 && (
          <span className="text-xs shrink-0" style={{ color: '#52525b' }} title="has dependencies">
      ...
      {expanded && !editing && (
        <div className="px-4 pb-2 pt-1 text-xs space-y-1" style={{ background: '#111113' }}>
          {task.description ? (
            <p className="leading-relaxed whitespace-pre-wrap" style={{ color: '#a1a1aa' }}>{task.description}</p>
          ) : (
            <p className="italic" style={{ color: '#52525b' }}>no description</p>
          )}
          <div className="flex gap-4" style={{ color: '#71717a' }}>
            {task.deadline && <span>deadline: <span className="text-amber-400">{task.deadline.slice(0, 10)}</span></span>}
            {task.recurrence_rule && <span>recurs: <span style={{ color: '#a1a1aa' }}>{task.recurrence_rule}</span></span>}
            <span>source: {task.source}</span>
          </div>
        </div>
      )}
      {editing && (
        <div className="px-4 pb-2 pt-1.5 text-xs space-y-1.5" style={{ background: '#111113', borderTop: '1px solid #27272a' }}>
          <input
            autoFocus
          ...
          {projectTasks.length > 0 && (
            <div>
              <span style={{ color: '#71717a' }}>blocks on:</span>
              <div className="flex flex-wrap gap-2 mt-1">
                {projectTasks.map((pt) => (
                  <label key={pt.id} className="flex items-center gap-1 cursor-pointer">
                    ...
                    />
                    <span className="text-xs" style={{ color: pt.status === 'done' ? '#52525b' : '#a1a1aa', textDecoration: pt.status === 'done' ? 'line-through' : 'none' }}>
                      {pt.title}
                    </span>
                  </label>
                ))}
...
function AddTaskForm({ projectId, onDone }: { projectId: string; onDone: () => void }) {
  ...
  const fieldStyle = { background: '#18181b', border: '1px solid #27272a', color: '#f4f4f5' }

  return (
    <div className="px-4 py-2 text-xs space-y-1.5" style={{ background: '#111113', borderBottom: '1px solid #27272a' }}>
      <div className="flex items-center gap-2">
        <input
          autoFocus
        ...
        />
        <span style={{ color: '#71717a' }}>load</span>
        <select
          className={fieldCls}
          style={fieldStyle}
          value={load}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span style={{ color: '#71717a' }}>recurs</span>
        <select
          className={fieldCls}
          style={fieldStyle}
          value={recurrence}
        ...
        <button
          ...
          className="text-xs font-medium px-3 py-1.5 rounded-md transition-colors ml-auto"
          style={{ background: isPending || !title ? '#27272a' : '#18181b', color: '#a1a1aa', border: '1px solid #27272a' }}
          onMouseEnter={e => (e.currentTarget.style.background = '#3f3f46')}
          onMouseLeave={e => (e.currentTarget.style.background = isPending || !title ? '#27272a' : '#18181b')}
        >
          {isPending ? '…' : '+ Add task'}
        </button>
        <button
  ...
  return (
    <div className="border-b" style={{ borderColor: '#27272a' }}>
      <div
        ...
        onClick={() => setExpanded((v) => !v)}
      >
        <span className="text-xs w-3 shrink-0" style={{ color: '#52525b' }}>{expanded ? '▾' : '▸'}</span>
        <span className="font-medium flex-1" style={{ color: project.status === 'done' ? '#52525b' : '#e4e4e7', textDecoration: project.status === 'done' ? 'line-through' : 'none' }}>
          {project.title}
        </span>
        <span className={`text-xs w-24 shrink-0 ${CAT_COLORS[project.category] ?? 'text-zinc-400'}`}>
          {project.category}
        </span>
        <span className="text-xs font-mono w-16 text-right shrink-0" style={{ color: '#71717a' }}>
          {project.estimated_hours_remaining.toFixed(0)}h left
        </span>
        <span className="text-xs font-mono w-14 text-right shrink-0" style={{ color: '#52525b' }}>
          {project.priority_score.toFixed(2)}
        </span>
        <span className="text-xs px-2 py-0.5 rounded-full w-16 text-center shrink-0"
          style={STATUS_PILL[project.status] ?? { background: '#27272a', color: '#71717a' }}>
          {project.status}
        </span>
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity text-xs w-28 justify-end shrink-0">
          {project.status === 'active' && (
            <>
              <button className="transition-colors" style={{ color: '#71717a' }}
                onMouseEnter={e => (e.currentTarget.style.color = '#a1a1aa')}
                onMouseLeave={e => (e.currentTarget.style.color = '#71717a')}
      ...
      {expanded && (
        <div style={{ background: '#09090b' }}>
          <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: '#27272a' }}>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={syncing}
                className="text-xs transition-colors"
                style={{ color: syncing ? '#3f3f46' : '#71717a' }}
          ...
          {tasks.length === 0 && !addingTask ? (
            <div className="px-4 py-3 text-xs" style={{ color: '#52525b' }}>no tasks</div>
          ) : (
            tasks.map((t) => (
              <TaskRow
                key={t.id}
...
function AddProjectForm({ goals, onDone }: { goals: Goal[]; onDone: () => void }) {
  ...
  return (
    <div className="px-6 py-3 border-b text-xs" style={{ background: '#111113', borderColor: '#27272a' }}>
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span style={{ color: '#71717a' }}>hrs</span>
        <input
          className={`w-14 ${fieldCls}`}
          style={fieldStyle}
          value={hours}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b shrink-0" style={{ borderColor: '#27272a' }}>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 300, color: '#f4f4f5', letterSpacing: '-0.02em' }}>Projects</span>
      ...
      <div className="flex items-center gap-3 px-6 py-1.5 text-xs border-b shrink-0" style={{ color: '#3f3f46', borderColor: '#27272a' }}>
        <span className="w-3" />
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          <div className="px-6 py-4 text-xs" style={{ color: '#52525b' }}>loading...</div>
        ) : projects.length === 0 && !adding ? (
          <div className="px-6 py-8 text-xs" style={{ color: '#52525b' }}>no projects — add one above.</div>
        ) : (
          projects.map((p) => (
            <ProjectCard
              key={p.id}
