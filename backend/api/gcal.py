...
from backend.integrations.gcal import GCalClient
...
def _ensure_fresh_token(token: GCalToken, db: Session) -> GCalClient:
...
def _sync_gcal(db: Session) -> dict:
    ...
    for event in events:
        ...
        if not start_str or not end_str:
            # Skip all-day events (only have start.date, not start.dateTime)
            continue
    ...
    # Export Eden blocks back to GCal
    ...
    for block in eden_blocks:
        ...
        try:
            ...
            pass  # Best-effort export — don't fail the whole sync
...
def gcal_sync(db: Session = Depends(get_db)):
    token = db.query(GCalToken).first()
    if not token:
        raise HTTPException(status_code=400, detail="GCal not connected. Visit /api/gcal/connect first.")
    return _sync_gcal(db)
